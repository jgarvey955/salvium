...
namespace
{
    ...
    unsigned char recipient_tag = 2)
  {
    ...
    std::memset(&e.recipient_tag,recipient_tag,sizeof(e.recipient_tag));
...
TEST(salchat, repeated_polls_advance_without_acknowledgements)
{
  cryptonote::salchat_relay relay;
  std::string error;
  crypto::hash first_tag{}, second_tag{}, last_id{};
  // Two recipient epochs under the default per-tag limit can exceed one batch.
  for (unsigned int i = 0; i < 110; ++i)
  {
    const auto envelope = valid_envelope(1000, "opaque-" + std::to_string(i),
      nullptr, i < 55 ? 2 : 3);
    ASSERT_EQ(relay.insert(envelope, 1000, error), cryptonote::salchat_result::accepted) << error;
    (i < 55 ? first_tag : second_tag) = envelope.recipient_tag;
    last_id = envelope.message_id;
  }
  const auto first = relay.poll({first_tag, second_tag}, 100, 1000);
  ASSERT_EQ(first.size(), 100u);
  EXPECT_TRUE(std::none_of(first.begin(), first.end(), [&](const auto& e) { return e.message_id == last_id; }));
  const auto unrelated = valid_envelope(1000, "another recipient", nullptr, 4);
  ASSERT_EQ(relay.insert(unrelated, 1000, error), cryptonote::salchat_result::accepted);
  ASSERT_EQ(relay.poll({unrelated.recipient_tag}, 100, 1000).size(), 1u);
  // Tag ordering and duplicates must not reset a query's progress either.
  const auto second = relay.poll({second_tag, first_tag, first_tag}, 100, 1000);
  EXPECT_TRUE(std::any_of(second.begin(), second.end(), [&](const auto& e) { return e.message_id == last_id; }));
  EXPECT_EQ(relay.statistics().cached_messages, 111u);
  EXPECT_FALSE(relay.ack(last_id, crypto::null_hash));
  EXPECT_TRUE(relay.ack(last_id, valid_ack_token()));
  EXPECT_EQ(relay.poll({first_tag, second_tag}, 100, 1000).size(), 100u);
}
...
TEST(salchat, recipient_quota_evicts_oldest_instead_of_locking_delivery)
{
  ...
  EXPECT_FALSE(relay.ack(first.message_id, valid_ack_token()));
  EXPECT_TRUE(relay.ack(third.message_id, valid_ack_token()));
  EXPECT_TRUE(relay.ack(second.message_id, valid_ack_token()));
  EXPECT_EQ(relay.statistics().cached_bytes, 0u);
}
...
TEST(salchat, ack_index_survives_growth_and_middle_removal)
{
  cryptonote::salchat_config config;
  config.max_per_recipient = config.max_per_sender_recipient = 128;
  cryptonote::salchat_relay relay{config};
  const test_signer signer;
  std::vector<cryptonote::salchat_p2p_envelope> envelopes;
  std::vector<std::size_t> sizes;
  std::size_t remaining_bytes = 0;
  std::string error;
  for (unsigned int i = 0; i < 128; ++i)
  {
    auto envelope = valid_envelope(1000 + i, "indexed-" + std::to_string(i), &signer);
    epee::byte_slice wire;
    ASSERT_TRUE(epee::serialization::store_t_to_binary(envelope, wire));
    sizes.push_back(wire.size());
    remaining_bytes += wire.size();
    ASSERT_EQ(relay.insert(envelope, 1127, error), cryptonote::salchat_result::accepted) << error;
    envelopes.push_back(std::move(envelope));
  }
  EXPECT_EQ(relay.statistics().cached_bytes, remaining_bytes);
  EXPECT_FALSE(relay.ack(crypto::null_hash, valid_ack_token()));
  for (unsigned int i = 0; i < 128; ++i)
  {
    const auto index = (i * 73) % 128; // Visit every entry, away from insertion order.
    const auto& envelope = envelopes[index];
    EXPECT_FALSE(relay.ack(envelope.message_id, crypto::null_hash));
    ASSERT_TRUE(relay.ack(envelope.message_id, valid_ack_token()));
    EXPECT_FALSE(relay.ack(envelope.message_id, valid_ack_token()));
    remaining_bytes -= sizes[index];
    EXPECT_EQ(relay.statistics().cached_messages, 127 - i);
    EXPECT_EQ(relay.statistics().cached_bytes, remaining_bytes);
  }
  EXPECT_TRUE(relay.poll({envelopes.front().recipient_tag}, 100, 1127).empty());
}

TEST(salchat, ack_index_tracks_global_count_and_byte_eviction)
{
  for (const bool byte_limit : {false, true})
  {
    auto first = valid_envelope(1000, "first");
    auto second = valid_envelope(1000, "other");
    auto third = valid_envelope(1000, "third");
    epee::byte_slice wire;
    ASSERT_TRUE(epee::serialization::store_t_to_binary(first, wire));
    cryptonote::salchat_config config;
    if (byte_limit) config.max_cache_bytes = 2 * wire.size();
    else config.max_cache_messages = 2;
    cryptonote::salchat_relay relay{config};
    std::string error;
    for (const auto* envelope : {&first, &second, &third})
      ASSERT_EQ(relay.insert(*envelope, 1000, error), cryptonote::salchat_result::accepted) << error;
    EXPECT_EQ(relay.statistics().cached_messages, 2u);
    EXPECT_EQ(relay.statistics().evicted, 1u);
    EXPECT_FALSE(relay.ack(first.message_id, valid_ack_token()));
    EXPECT_TRUE(relay.ack(second.message_id, valid_ack_token()));
    EXPECT_TRUE(relay.ack(third.message_id, valid_ack_token()));
...
TEST(salchat, ack_index_tracks_pruning_and_removed_poll_cursors)
{
  cryptonote::salchat_relay relay;
  const auto first = valid_envelope(1000);
  const auto second = valid_envelope(1001);
  std::string error;
  ASSERT_EQ(relay.insert(first, 1001, error), cryptonote::salchat_result::accepted);
  ASSERT_EQ(relay.insert(second, 1001, error), cryptonote::salchat_result::accepted);
  auto batch = relay.poll({first.recipient_tag}, 1, 1001);
  ASSERT_EQ(batch.size(), 1u);
  EXPECT_EQ(batch.front().message_id, first.message_id);
  batch = relay.poll({first.recipient_tag}, 1, first.expires_at);
  ASSERT_EQ(batch.size(), 1u);
  EXPECT_EQ(batch.front().message_id, second.message_id);
  EXPECT_FALSE(relay.ack(first.message_id, valid_ack_token()));
  EXPECT_TRUE(relay.ack(second.message_id, valid_ack_token()));
  EXPECT_TRUE(relay.poll({first.recipient_tag}, 1, first.expires_at).empty());

  const auto next = valid_envelope(first.expires_at);
  ASSERT_EQ(relay.insert(next, first.expires_at, error), cryptonote::salchat_result::accepted);
  EXPECT_EQ(relay.poll({next.recipient_tag}, 1, first.expires_at).size(), 1u);
  EXPECT_TRUE(relay.poll({next.recipient_tag}, 1, first.expires_at + 1, next.expires_height).empty());
  EXPECT_FALSE(relay.ack(next.message_id, valid_ack_token()));
  EXPECT_EQ(relay.statistics().cached_bytes, 0u);
}
