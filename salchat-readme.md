...
- time and block-height expiration, acknowledgement removal, and oldest-first eviction.
...
This prints the complete contact code (`SC_address:encryption_key`) and the
command another person can use to add you. Share the entire code, including
the colon and public encryption key. The Salvium address alone is not enough
to add a SalChat contact. The other person chooses a label for you.

To print just the contact code, use `salchat address`.
