...
namespace cryptonote
{
  ...
  void salchat_relay::remove_entry_accounting(const entry& value)
  {
    ...
    m_entries_by_id.erase(key(value.envelope.message_id));
    ...
    const std::uint64_t current_height, std::string& error)
  {
    ...
    if (m_entries_by_id.count(id_key) || m_active_hashes.count(ciphertext_key) ||
    ...
    try
    {
      m_entries_by_id.emplace(id_key, std::prev(m_entries.end()));
    }
    catch (...)
    {
      m_entries.pop_back();
    ...
    m_active_hashes.emplace(ciphertext_key);
    ...
    std::size_t limit, std::uint64_t now, std::uint64_t current_height)
  {
    ...
    std::vector<std::string> ordered_tags(wanted.begin(), wanted.end());
    std::sort(ordered_tags.begin(), ordered_tags.end());
    std::string query;
    for (const auto& tag: ordered_tags) query.append(tag);
    const std::string query_key = key(crypto::cn_fast_hash(query.data(), query.size()));
    std::lock_guard<std::mutex> lock(m_mutex); prune(now, current_height);
    std::vector<salchat_p2p_envelope> out;
    // Polling is non-destructive. Advance past the previous batch even when a
    // receiver cannot acknowledge its contents, and wrap after the last entry.
    // Keep insertion order intact for cache eviction. Unrelated recipient
    // queries have independent cursors so they cannot move each other's poll.
    const auto cursor = m_poll_cursors.find(query_key);
    const auto indexed = cursor == m_poll_cursors.end() ? m_entries_by_id.end() :
      m_entries_by_id.find(key(cursor->second));
    const auto start = indexed == m_entries_by_id.end() ? m_entries.begin() : std::next(indexed->second);
    const auto append = [&](auto first, auto last) {
      for (; first != last && out.size() < limit; ++first)
        if (wanted.count(key(first->envelope.recipient_tag)))
          out.push_back(first->envelope);
    };
    append(start, m_entries.end());
    append(m_entries.begin(), start);
    if (!out.empty())
    {
      if (cursor == m_poll_cursors.end())
      {
        constexpr std::size_t max_poll_cursors = 1024;
        if (m_poll_cursors.size() >= max_poll_cursors)
        {
          m_poll_cursors.erase(m_poll_cursor_order.front());
          m_poll_cursor_order.pop_front();
        }
        m_poll_cursor_order.push_back(query_key);
      }
      m_poll_cursors[query_key] = out.back().message_id;
    }
  ...
  {
    std::lock_guard<std::mutex> lock(m_mutex);
    const auto found = m_entries_by_id.find(key(id));
    if (found == m_entries_by_id.end())
      return false;
